...
export default function DelegateButton({ newDelegateeAddr }: { newDelegateeAddr: `0x${string}` }) {
	...
	const isCurrentDelegate = delegateAddress ? (newDelegateeAddr.toLowerCase() === delegateAddress.toLowerCase()) : false

	const burnAddress = new RegExp('0x0000000000')
	const hasDelegate = delegateAddress ? !(burnAddress.test(delegateAddress)) : false
